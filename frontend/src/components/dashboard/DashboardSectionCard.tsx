...
const compactTime = (value?: string) => value?.replaceAll(':00', '') ?? ''

export function DashboardSectionCard({ section }: { section: DashboardSection }) {
  const Icon = icons[section.kind]
  return <Card className="h-full"><div className="flex items-start gap-3"><span className="grid size-9 shrink-0 place-items-center rounded bg-primary/10 text-primary"><Icon aria-hidden="true" className="size-4" /></span><div className="min-w-0"><h2 className="font-bold text-text">{section.title}</h2>{section.description && <p className="mt-0.5 text-sm text-muted">{section.description}</p>}</div></div>{section.items.length === 0 ? <div className="mt-5"><EmptyState title={section.emptyTitle} description={section.emptyDescription} /></div> : <ul className="mt-5 divide-y divide-border">{/* Title and detail do not identify a row: "Today's timetable" repeats a subject in the same
...
{section.items.map((item, index) => <li key={`${index}-${item.title}-${item.detail ?? ''}`} className="py-3 first:pt-0 last:pb-0">{section.kind === 'schedule' ? <div className="grid grid-cols-[minmax(0,1fr)_8.75rem] items-start gap-3"><div className="min-w-0"><p className="font-semibold leading-6 text-text">{item.title}</p>{item.detail && <p className="mt-0.5 text-sm leading-5 text-muted">{item.detail}</p>}</div>{item.value && <span className="justify-self-end whitespace-nowrap rounded-full bg-background px-3 py-1 text-center text-xs font-bold leading-5 text-primary shadow-inner ring-1 ring-border sm:text-sm">{compactTime(item.value)}</span>}</div> : <div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="font-semibold text-text">{item.title}</p>{item.detail && <p className="mt-0.5 text-sm leading-5 text-muted">{item.detail}</p>}</div>{item.value && section.kind !== 'progress' && <Badge tone={badgeTones[item.tone ?? 'primary']}>{item.value}</Badge>}</div>}{section.kind === 'progress' && <div className="mt-3"><div className="flex justify-between gap-3 text-xs font-semibold text-muted"><span>Completion</span><span>{item.value ?? `${item.progress ?? 0}%`}</span></div><div className="mt-1.5 h-2 overflow-hidden rounded-full bg-background"><div aria-label={`${item.title} progress`} className={`h-full rounded-full ${toneClasses[item.tone ?? 'primary']}`} style={{ width: `${item.progress ?? 0}%` }} /></div></div>}</li>)}</ul>}</Card>
}
