...
export function AuditPage() {
  ...
  const { records, profiles, context, references } = resource.data
  const recordActors = new Map(profiles.map((profile) => [profile.id, profile]))
  // Names every id the audit rows point at; see auditReferences in the repository.
  const resolveReference = (id: string) => references.get(id) ?? null
