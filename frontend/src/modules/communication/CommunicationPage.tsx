...
export function CommunicationPage({ sections = allSections }: { sections?: CommunicationSection[] } = {}) {
  ...
  // action (and the RLS write behind it) is Super Admin-exclusive.
  const canViewComplaints = canSubmitComplaint || canReviewComplaints || role === USER_ROLES.hod
  ...
  return <div className="space-y-6">
    ...
    <Card><div className={`grid gap-3 ${shows('announcements') && role !== USER_ROLES.student ? 'md:grid-cols-[1fr_12rem]' : ''}`}><label className="relative"><Search className="pointer-events-none absolute left-3 top-3 size-4 text-muted" /><Input className="pl-9" value={query} onChange={(event) => setQuery(event.target.value)} placeholder={heading.search} /></label>{shows('announcements') && role !== USER_ROLES.student && <Select value={filter} onChange={(event) => setFilter(event.target.value)}><option value="all">All announcement audiences</option>{audiences.map((audience) => <option key={audience} value={audience}>{audience.replace('_', ' ')}</option>)}</Select>}</div></Card>
