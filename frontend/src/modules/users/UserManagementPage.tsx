...
function UserManagementContent() {
  ...
  const [status, setStatus] = useState<Database['public']['Enums']['user_status']>('active')
  ...
  const users = data.profiles.filter((user) =>
    ...
    `${user.full_name} ${user.email} ${user.employee_or_register_number ?? ''}`.toLowerCase().includes(query.toLowerCase()))
  const department = (id: string | null) => data.departments.find((item) => item.id === id)?.name ?? '-'

  const open = (user: Profile) => {
    if (!canManage) return
    setEditing(user)
  ...
  const save = async () => {
    ...
    if (!editing || name.trim().length < 2) {
      setMessage('Enter a valid profile name.')
      return
    }
    setSaving(true)
    setMessage('')
    try {
      await academicRepository.updateProfile(editing.id, { full_name: name.trim(), status, department_id: departmentId || null })
      await resource.reload()
      setEditing(null)
    } catch (reason) {
      setMessage(reason instanceof Error ? reason.message : 'Unable to update profile.')
    } finally {
      setSaving(false)
    }
  }
  ...
  return <div className="space-y-6">
    <PageHeader
      ...
      description={canManage ? 'Manage existing authenticated profiles; creating Auth accounts remains a controlled bootstrap task.' : 'Monitor existing authenticated profiles and department account status.'}
      actions={<Button variant="secondary" onClick={() => void resource.reload()}><RefreshCw className="size-4" /> Refresh</Button>}
    />
    <Card>
      <div className="grid gap-3 md:grid-cols-[1fr_12rem]">
        <label className="relative">
          ...
          <Input className="pl-9" placeholder="Search profiles" value={query} onChange={(event) => setQuery(event.target.value)} />
        </label>
      ...
      <div className="mt-5"><DataTable rows={users} empty={<EmptyState title="No matching profiles" />} columns={[
        { header: 'Profile', render: (user) => <div><p className="font-semibold">{user.full_name}</p><p className="text-xs text-muted">{user.email}</p></div> },
        { header: 'Role / ID', render: (user) => <div><p>{user.role.replaceAll('_', ' ')}</p><p className="text-xs text-muted">{user.employee_or_register_number ?? '-'}</p></div> },
        ...
        ...(canManage ? [{ header: 'Actions', render: (user: Profile) => <Button aria-label={`Edit ${user.full_name}`} className="min-h-8 px-2" variant="ghost" onClick={() => open(user)}><Pencil className="size-4" /></Button> }] : []),
      ]} /></div>
    </Card>
    <Modal isOpen={Boolean(editing)} title="Edit existing profile" onClose={() => setEditing(null)}>
