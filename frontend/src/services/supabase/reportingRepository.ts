import { formatAuditMetadata, type ReferenceResolver } from '@/lib/audit-metadata'
...
const dateInRange = (date: string, filters: ReportFilters) => (!filters.fromDate || date >= filters.fromDate) && (!filters.toDate || date <= filters.toDate)
/** Audit metadata references rows by UUID; show the names the viewer is allowed to see. */
const referenceResolver = (data: ReportingData): ReferenceResolver => (id) =>
  data.profiles.find((profile) => profile.id === id)?.full_name
  ?? data.subjects.find((subject) => subject.id === id)?.name
  ?? data.sections.find((section) => section.id === id)?.name
  ?? null
const safeMetadata = (data: ReportingData, value: Json | null) => formatAuditMetadata(value, referenceResolver(data))
...
export const reportingRepository = {
  ...
  dashboard(data: ReportingData): RoleDashboardData {
    ...
    const quickActions =
      user.role === 'super_admin' ? [
        { label: 'Users', description: 'Review department accounts', path: `${ROUTE_PATHS.superAdmin}/users` },
        { label: 'Academic setup', description: 'Manage years, sections, and subjects', path: `${ROUTE_PATHS.superAdmin}/academic-setup` },
        { label: 'Reports', description: 'Filter operational reports', path: `${ROUTE_PATHS.superAdmin}/reports` },
        { label: 'Audit', description: 'Inspect recorded events', path: `${ROUTE_PATHS.superAdmin}/audit` },
      ] : user.role === 'hod' ? [
        { label: 'Approvals', description: 'Review leave, gate pass, and OD requests', path: `${ROUTE_PATHS.hod}/requests` },
        { label: 'Attendance', description: 'Review department attendance', path: `${ROUTE_PATHS.hod}/attendance` },
        { label: 'Reports', description: 'Filter operational reports', path: `${ROUTE_PATHS.hod}/reports` },
        { label: 'Announcements', description: 'Publish department updates', path: `${ROUTE_PATHS.hod}/announcements` },
      ] : user.role === 'faculty' ? [
        { label: 'Mark attendance', description: 'Open today\'s class sheet', path: `${ROUTE_PATHS.faculty}/attendance` },
        { label: 'Enter marks', description: 'Save drafts and finalize assessments', path: `${ROUTE_PATHS.faculty}/marks` },
        { label: 'Review requests', description: 'Handle student workflow items', path: `${ROUTE_PATHS.faculty}/requests` },
        { label: 'Announcements', description: 'Message assigned students', path: `${ROUTE_PATHS.faculty}/announcements` },
      ] : user.role === 'lab_assistant' ? [
        { label: 'Lab timetable', description: 'See assigned lab periods', path: `${ROUTE_PATHS.labAssistant}/lab-timetable` },
        { label: 'Attendance', description: 'Check in and view records', path: `${ROUTE_PATHS.labAssistant}/attendance` },
        { label: 'Leave', description: 'Submit staff leave requests', path: `${ROUTE_PATHS.labAssistant}/leave` },
        { label: 'Announcements', description: 'Read department updates', path: `${ROUTE_PATHS.labAssistant}/announcements` },
      ] : []
    if (user.role === 'super_admin') return {
      role: user.role, title: 'Department overview', description: 'Live department metrics across every record your administrator role can see.', quickActions,
    ...
    if (user.role === 'hod') {
      ...
      return { role: user.role, title: 'Department dashboard', description: 'Department attendance, academic progress, and faculty workload.', quickActions,
  ...
  rows(data: ReportingData, kind: ReportKind, filters: ReportFilters): ReportRow[] {
    ...
    if (kind === 'audit') return data.auditLogs.filter((log) => filterCommon(null, null, log.actor_id, log.action, log.created_at)).map((log) => ({ id: log.id, reference: name(data, log.actor_id, 'System'), metric: `${log.action} · ${log.module}`, value: log.record_reference, state: log.actor_role ?? 'system', detail: `${new Date(log.created_at).toLocaleString()} · ${safeMetadata(data, log.after_data ?? log.before_data)}` }))
